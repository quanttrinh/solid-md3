...
function AccordionItem(props: AccordionItemProps) {
  const [local, rest] = splitProps(props, ['value', 'title', 'description', 'disabled', 'class', 'children']);
    ...
    >
      ...
      >
        <div class="flex flex-1 flex-col gap-0.5">
          <div class="text-md3-title-md text-md3-on-surface font-medium">{local.title}</div>
          <Show when={local.description}>
            <div class="text-md3-body-sm text-md3-on-surface-variant">{local.description}</div>
          </Show>
        </div>
        <ArkAccordion.ItemIndicator class="text-md3-on-surface-variant transition-transform duration-200 data-[state=open]:rotate-180">
          <span class="material-symbols-outlined text-lg">expand_more</span>
        </ArkAccordion.ItemIndicator>
      </ArkAccordion.ItemTrigger>
      <ArkAccordion.ItemContent
        class="data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out data-[state=open]:fade-in px-4 pb-4 pt-1"
      >
