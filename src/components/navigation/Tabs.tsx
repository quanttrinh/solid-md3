...
function TabsRoot(props: TabsRootProps) {
  const [local, rest] = splitProps(props, ['value', 'defaultValue', 'onValueChange', 'orientation', 'class', 'children']);
...
function TabsList(props: JSX.HTMLAttributes<HTMLDivElement> & { class?: string }) {
...
function TabsTrigger(props: TabsTriggerProps) {
  ...
  return (
    <ArkTabs.Trigger
      ...
      class={cn(
        'state-layer text-md3-label-lg border-md3-primary -mb-px border-b-2 border-transparent',
...
function TabsContent(props: TabsContentProps) {
  ...
  return (
    <ArkTabs.Content
      value={local.value}
      class={cn('data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=open]:fade-in data-[state=closed]:fade-out text-md3-body-md text-md3-on-surface', local.class)}
      {...rest}
    >
      {local.children}
    </ArkTabs.Content>
